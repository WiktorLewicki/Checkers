...
class place{
	...
	int dama_ruch(int a, int b, int c, int d, int r1, int r2, bool gracz, bool oblig){//ruch damą
		...
		if(pn1==-1){
			print();
			if(!who) cout<<"Nie mozesz zrobic takiego ruchu za pomoca damy\n";
			stan=0;
			return 0;
		}
		else{
			if(pn1) bicie(pn1, pn2);
				...
				print();
				if(!who) cout<<"Musisz wykonac bicie\n";
				stan=0;
				return 0;
			}
			move(a, b, c, d, gracz);
	...
	bool cont(){
		if(stan==2) bierki[!tura]--;
		if(bierki[0]==0){
			if(!who_pom) cout<<"Wygral gracz 2\n";
			return true;
		}
		if(bierki[1]==0){
			if(!who) if(!who_pom) cout<<"Wygral gracz 1\n";
			return true;
		}
		...
		if(czy_koniec()){
			if(tura==0&&!who_pom) cout<<"Wygral gracz 2!\n"; 
			else if(!who_pom) cout<<"Wygral gracz 1!\n";
			return true;
		}
		return false;
	}
	int ruch_gracza(bool bt=false, int m1=0, int m2=0, int m3=0, int m4=0, bool show=true){
		who=bt;
		who_pom=show;
		if(cont()) return -1;
		bool gracz=tura;
		if(gracz==0&&!who) cout<<"rusza sie gracz 1\n";
		else if(!who) cout<<"rusza sie gracz 2\n";
		int a, b, c, d;
		if(!who) cout<<"Zaznacz pionka ktorym chcesz ruszyc\n";
		if(!bt) cin>>a>>b;
		else a=m1, b=m2;
		if(last1&&last2){//kontynuowanie bicia
			if(a!=last1||b!=last2){
				print();
				if(!who) cout<<"Musisz kontynuowac bicie tym pionkiem ktorym sie ruszyles, pole: "<<last1<<" "<<last2<<"\n";
				stan=0;
				return 0;
			}
		}
		if(!gd(a, b)){//sprawdzanie czy poza planszą
			print();
			if(!who) cout<<"Nieprawidlowe dane\n";
			stan=0;
			return 0;
		}
		if(!comp(a, b, gracz)){//sprawdzanie czy na polu jest bierek gracza
			print();
			if(!who) cout<<"Tu nie ma twojego pionka\n";
			stan=0;
			return 0;
		}
		if(!who) cout<<"Podaj pole na ktorym chcesz postawic tego pionka\n";
		if(!bt) cin>>c>>d;
		else c=m3, d=m4;
		...
		if(p[c][d]!=5){//sprawdzanie czy pole puste
			print();
			if(!who) cout<<"Na tym polu stoi inny pionek\n";
			stan=0;
			return 0;
		}
		...
		if((c==a-2*dr&&d==b-2)||(c==a-2*dr&&d==b+2)){//bicie pionkiem
			if(c==a-2*dr&&d==b-2){
				if(comp(a-1*dr, b-1, !gracz)){
					...
					print();
					if(!who) cout<<"Nie mozesz zrobic takiego ruchu\n";
					stan=0;
					return 0;
				}
			}
